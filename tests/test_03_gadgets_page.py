...
def test_guest_can_follow_the_whole_purchase_root(browser):
    """
    В данном тесте добавляется товар в корзину. Проверка во вкладке 'Корзина' по названию и стоимости товара,
    что добавился именно тот товар, что был выбран в 'Гаджеты'. Увеличивается его количество на 1.
    Проверяется, что стоимость увеличилась x2. Переход на страницу оформления заказа. Проверка,
    ...
    Заполнение полей с именем и телефоном. Проверка, что данные совпали.
    Возврат на предыдущую страницу. Очистка корзины. Сохранение скриншотов.
    """
